...
!    use mpi
    ...
    character (len=MPI_MAX_PROCESSOR_NAME):: nname
end module
!*********************
module getit
contains
...
! do the mpi init stuff
    ...
    call MPI_Get_processor_name(nname,nlen,mpi_err)
...
! we stripe the grid across the processors
    ...
    write(*,101)myid,trim(nname),get_core_c(),i1,i2,j1,j2
101 format("myid= ",i4," node= ",a15," core=",i3,3x,&
           " (",i3," <= i <= ",i3,") , ",            &
           " (",i3," <= j <= ",i3,")")
...
!for this example we supress most output
