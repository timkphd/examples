...
def main():
    ...
    dataset = dataset.repeat().shuffle(mysize).batch(128)
    ...
    for batch, (images, labels) in enumerate(dataset.take(mysize // hvd.size())):
        loss_value = training_step(images, labels, batch == 0)

        if batch % 10 == 0 and hvd.rank() == 0:
            print('Step #%d\tLoss: %.10f' % (batch, loss_value))
