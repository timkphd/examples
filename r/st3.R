...
for(i in 1:(numprocs)) {
	...
	tmp <- tmpData[inds, ]
	if(i> 1){
		mpi.send.Robj(tmp, i-1, 1234, comm = mpi_comm_world)
		print(c("sending to",i-1,dim(tmp)))
	}
}
	i<-1
	inds <- seq(from=(i-1)*ceiling(nrow(tmpData)/nCores)+1, to=min(i*ceiling(nrow(tmpData)/nCores),nrow(tmpData)), by=1)
...
while(mpi.iprobe(0, 1234, comm = mpi_comm_world, status)==FALSE){
	...
	tmpData<-mpi.recv.Robj(0, 1234, comm = mpi_comm_world, status = 0)
	print(c(myid,"got it"))

}
	z<-paste("b",myid,sep="_")
