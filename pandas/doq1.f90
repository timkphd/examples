...
!
     ...
     select case (myset)
     case (1)
        CALL h5dcreate_f(file_id, dsetname1, H5T_NATIVE_INTEGER, dataspace(myset), dset_id(myset), error)
        CALL h5dwrite_f(dset_id(myset), H5T_NATIVE_INTEGER, buf1, data_dims, error)
     case(2)
         CALL h5dcreate_f(file_id, dsetname2, H5T_NATIVE_INTEGER, dataspace(myset), dset_id(myset), error)
