...
dep=q['depth']
print("one",type(dep),dep)
dep=np.array(dep)
print("two",type(dep),dep)
dep=pd.DataFrame(dep)
print("three",type(dep),dep)
#dep=np.array(dep)
#print(type(dep),dep)

#dep=np.array(dep['depth'],dtype=float)

lat=q['latitude']
lat=np.array(lat)
lat=pd.DataFrame(lat)
lat=np.array(lat)
#lat=np.array(lat['latitude'],dtype=float)
...
if True :
    ...
    lon=[np.array(lon['longitude'],dtype=float)]
print(lon)

mymag=q['SCSN']
mymag=np.array(mymag)
mymag=pd.DataFrame(mymag)
...
for mylat in lat_seq:
    ...
    for mylon in lon_seq:
        ...
        #tymer(["-i","got dist"])
        #tin=vatin(dist)
#        result=vatin(whack4b(mylat,mylon,lat['latitude'],lon['longitude']))*mymag
