...
def myplot(ismain=False,files=["dummy_file_name"],bl="x axis",sl="y axis",topl="title",dl="none",xr="auto",yr="auto",ds="n",width="1",doxkcd=False,sets=None):
#sets=[[[1,2,3,4],[1,4,9,16],"square"],[[1,2,3],[1,16,27],"qube"]]
    if len(sys.argv) > 1:
...
def myplot(ismain=False,files=["dummy_file_name"],bl="x axis",sl="y axis",topl="title",dl="none",xr="auto",yr="auto",ds="n",width="1",doxkcd=False,sets=None):
#sets=[[[1,2,3,4],[1,4,9,16],"square"],[[1,2,3],[1,8,27],"qube"]]
    if len(sys.argv) > 1:
        if (sys.argv[1] == "--help" or sys.argv[1] == "-help" or sys.argv[1] == "-h"):
            htext=htext.replace("CMD",sys.argv[0])
            print(htext)
