...
INT numnodes,myid,mpi_err,numprocs;
#define mpi_master 0
INT myrow,mycol;
INT nrow,ncol;
INT myrow,mycol;
...
int my_select(int i, int steps) {

	if(steps == i*1000)return 1;
	if(steps == i*500)return 1;
	if(steps == i*250)return 1;
	if(steps == i*200)return 1;
	if(steps == i*100)return 1;
	if(steps == i*50)return 1;
	if(steps == i*25)return 1;
	if(steps == i*10)return 1;
	if(steps == i*5)return 1;
	if(steps == i*2)return 1;
	if(steps == i*1)return 1;
	return 0;
}
...
/* that will contain a subset of the  processors */
		...
		for (int ijk=0;ijk <= num_used-1;ijk++){
			will_use[ijk]=ijk;
		}
        mpi_err =  MPI_Group_incl(old_group,num_used,will_use,&new_group);
/* create the new communicator */
        mpi_err =  MPI_Comm_create(MPI_COMM_WORLD,new_group,&THE_WORLD);
}
#else
        mpi_err = MPI_Comm_dup( MPI_COMM_WORLD, &THE_WORLD );
#endif

    mpi_err=MPI_Comm_size(THE_WORLD,&numnodes);
    mpi_err=MPI_Comm_rank(THE_WORLD,&myid);
/*
...
*/
  mpi_err=MPI_Comm_split(THE_WORLD,myrow,mycol,&ROW_COMM);
...
/* ! all processors with the same col will be in the same COL_COMM */
  mpi_err=MPI_Comm_split(THE_WORLD,mycol,myrow,&COL_COMM);
...
/* ! find id of neighbors using the communicators created above */
  ...
  myright = myid_row+1;if( myright  == nodes_row)myright = MPI_PROC_NULL;
    ...
    mpi_err=MPI_Bcast(&nx,   1,MPI_INT,   mpi_master,THE_WORLD);
    mpi_err=MPI_Bcast(&ny,   1,MPI_INT,   mpi_master,THE_WORLD);
    mpi_err=MPI_Bcast(&steps,1,MPI_INT,   mpi_master,THE_WORLD);
    mpi_err=MPI_Bcast(&lx,   1,MPI_DOUBLE,mpi_master,THE_WORLD);
    mpi_err=MPI_Bcast(&ly,   1,MPI_DOUBLE,mpi_master,THE_WORLD);
    mpi_err=MPI_Bcast(&alpha,1,MPI_DOUBLE,mpi_master,THE_WORLD);
    mpi_err=MPI_Bcast(&beta, 1,MPI_DOUBLE,mpi_master,THE_WORLD);
    mpi_err=MPI_Bcast(&gamma,1,MPI_DOUBLE,mpi_master,THE_WORLD);
	...
	lastdiff=1e30;
      if(steps > 0){
       for( i=1; i<=steps;i++) {
        ...
        mpi_err= MPI_Reduce(&mydiff,&diff,1,MPI_DOUBLE,MPI_SUM,mpi_master,THE_WORLD);
        mpi_err=MPI_Bcast(&diff,   1,MPI_DOUBLE,   mpi_master,THE_WORLD);
        if(i % iout == 0 && myid == mpi_master){
				printf("%8d %18.6e %10.3f\n",i,diff,MPI_Wtime()-t1);
        	}
//		if (my_select(i,steps) == 1){
			if(diff*fact < lastdiff) {
			//fact=fact*1.01;
			fact=pow(fact,1.15);
			if (fact > 50) fact=50;
			lastdiff=diff;
			if(myid == mpi_master)printf("plotting %d %g\n",i,fact);
...
*/
    mpi_err=MPI_Comm_rank(MPI_COMM_WORLD,&myid_w);
    mpi_err=MPI_Comm_size(MPI_COMM_WORLD,&numwriter);
    mpi_err=MPI_Bcast(&nx,   1,MPI_INT,   mpi_master,MPI_COMM_WORLD);
    mpi_err=MPI_Bcast(&ny,   1,MPI_INT,   mpi_master,MPI_COMM_WORLD);
    numwriter=numwriter-1;
    istart=i1;
    iend=i2;
    jstart=j1;
    jend=j2;
    ...
    j3=ny+1;
    if(myid_w == numwriter){
        f18=fopen(unique("out3c_"),"w");
        fprintf(f18,"0 <= i <= %d , 0 <= j <= %d\n",i3-i0,j3-j0);
        arow=vector(j0,j3);
    ...
    for( i=i0;i<=i3;i++){
       ...
       if(myid_w == numwriter){
	       for( j=j0;j<=j3;j++){
	           fprintf(f18,"%18.5f",arow[j]);
	           if(j != j3)fprintf(f18," ");
	       }
	       fprintf(f18,"\n");
       }
    }
     if(myid_w == numwriter)fclose(f18);
