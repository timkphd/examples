...
! end stuff for mkl library
    ...
    open(98,file='data/rcsout',position='append')
    open(99,file='data/cells',position='append')
    open(88,file='data/rcsout.bin',position='append',FORM='UNFORMATTED',access='stream')
    open(89,file='data/cells.bin',position='append', FORM='UNFORMATTED',access='stream')
...
!   read(12,*)xs,ys
    ...
    do nbox=1,nmax
      ...
      write(99,"(f5.1)",advance="no")ereal
      write(89)ereal
...
!     write(*,*)x(nbox),y(nbox),a(nbox),eps(nbox)
 ...
 200 nbox=nbox-1
    ...
    do n=1,nbox
    	...
    	!if(abs(h1(k*a(n))-h1_org(k*a(n))) .gt. 0.001 )write(*,*)"h",h1(k*a(n)),h1_org(k*a(n))
    enddo
    do  n=1,nbox
        !cn=(j*pi*k*a(n)/2.0_b4)*(eps(n)-1.0_b4)*j1(k*a(n))
        cn=(j*pi*k*a(n)/2.0_b4)*(eps(n)-1.0_b4)*BESSEL_J1(k*a(n))
        !if(abs(BESSEL_J1(k*a(n))-j1(k*a(n))).gt.0.001)write(*,*)BESSEL_J1(k*a(n)),j1(k*a(n))
        do  m=1,nbox
            if(n.eq.m)then
...
!       write(10,16)m,20.0_b4*log10(abs(es)/0.001_b4)
        ...
        write(10,'(g20.10)')20.0_b4*log10(abs(es)/0.001_b4)
        write(98,"(f10.3)",advance="no")20.0_b4*log10(abs(es)/0.001_b4)
        write(88)20.0_b4*log10(abs(es)/0.001_b4)
!       write(11)abs(es)
    enddo
    write(98,*)
    tend=ccm_time()
