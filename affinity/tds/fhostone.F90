...
! define the basic real type and pi (not used in this example)
   ...
   integer, parameter :: in4 = selected_int_kind(4)
end module
...
contains
   function get_core_c()
      ...
      interface
#ifdef USEFAST
         integer(c_int) function cfunc() BIND(C, NAME='get_core_number')
#else
         integer(c_long) function cfunc() BIND(C, NAME='sched_getcpu')
#endif
         USE ISO_C_BINDING, ONLY: c_long, c_char,c_int
         end function cfunc
      end interface
      get_core_c = cfunc()
   end function
...
program hello
   ...
   call MPI_Bcast(dt, 1, MPI_Integer, 0, MPI_COMM_WORLD, ierr)
   if (dt > 0) then
      nints = 100000
      t1 = MPI_Wtime()
      t2 = t1
   ...
   if (when > 1) then
      do i = 0, numprocs - 1
         call MPI_Barrier(MPI_COMM_WORLD, ierr)
         if (i == myid) then
            if (when == 3 .or. when == 2) call str_upr(name)
