...
!version: Uses MPI_Sendrecv for communication.
...
! MPI_GATHERV is then used to send the data
    ...
    j3=ny+1
    if(myid .eq. mpi_master)then
        open(18,file=unique("out5d_"),recl=max(80,15*((ny)+3)+2))
...
! input is the grid and the indices for the interior cells
    ...
    if(even(myid_row))then  
        call MPI_Sendrecv(psi(i1,:),  num_y,MPI_DOUBLE_PRECISION, mytop,10,&
                          psi(i1-1,:),num_y,MPI_DOUBLE_PRECISION, mytop,10,&
                          COL_COMM,status, mpi_err)
        call MPI_Sendrecv(psi(i2,:),  num_y,MPI_DOUBLE_PRECISION, mybot,10,&
                          psi(i2+1,:),num_y,MPI_DOUBLE_PRECISION, mybot,10,&
                          COL_COMM,status, mpi_err)
    else    ! we are on an odd row processor
        call MPI_Sendrecv(psi(i2,:),  num_y,MPI_DOUBLE_PRECISION, mybot,10,&
                          psi(i2+1,:),num_y,MPI_DOUBLE_PRECISION, mybot,10,&
                          COL_COMM,status, mpi_err)
        call MPI_Sendrecv(psi(i1,:),  num_y,MPI_DOUBLE_PRECISION, mytop,10,&
