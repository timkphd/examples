...
void mat_out(FILE *f,char *mname,FLT **mesh,INT i1,INT i2,INT j1,INT j2, INT nx, INT ny);
...
fprintf(fout,"block 2 mytop=%d mybot=%d\n",mytop,mybot);
    if(myid == mpi_master) {
        scanf("%d %d",&nx,&ny);
        scanf("%lg %lg",&lx,&ly);
...
fprintf(fout,"block 3 nx=%d ny=%d lx=%g ly=%g alpha=%g beta=%g gamma=%g\n",nx,ny,lx,ly,alpha,beta,gamma);
...
fprintf(fout,"block 4 nx=%d ny=%d lx=%g ly=%g alpha=%g beta=%g gamma=%g\n",nx,ny,lx,ly,alpha,beta,gamma);
...
/* calculate the constants for the calculations */
    ...
    a6=pi/(ly);
fprintf(fout,"block 5 a1=%g a2=%g a3=%g a4=%g a5=%g\n",a1,a2,a3,a4,a5);
...
fprintf(fout,"block 6 i1=%d i2=%d j1=%d j2=%d\n",i1,i2,j1,j2);
...
/* set boundary conditions */
    bc(psi,i1,i2,j1,j2);
mat_out(fout,"block 7 psi",psi,i1-1,i2+1,j1-1,j2+1,nx,ny);
    do_force(i1,i2,j1,j2);
// mat_out(fout,"block 8 the_for",the_for,i1-1,i2+1,j1-1,j2+1,nx,ny);
...
mat_out(fout,"block 10 psi",psi,i1-1,i2+1,j1-1,j2+1,nx,ny);
        do_jacobi(psi,new_psi,&mydiff,i1,i2,j1,j2);
fprintf(fout,"block 11 mydiff=%g\n",mydiff);
mat_out(fout,"block 12 psi",psi,i1-1,i2+1,j1-1,j2+1,nx,ny);
        do_transfer(psi,i1,i2,j1,j2);
mat_out(fout,"block 13 psi",psi,i1-1,i2+1,j1-1,j2+1,nx,ny);
        mpi_err= MPI_Reduce(&mydiff,&diff,1,MPI_DOUBLE,MPI_SUM,mpi_master,MPI_COMM_WORLD);
fprintf(fout,"block 14 mydiff=%g\n",mydiff);
...
void mat_out(FILE *f,char *mname,FLT **mesh,INT i1,INT i2,INT j1,INT j2, INT nx1, INT ny1){
	int i,j;
	int color;
	fprintf(f,"%s[%d:%d][%d:%d]\n",mname,i1,i2,j1,j2);
	for (j=j1;j<=j2;j++) {
		for (i=i1;i<=i2;i++) {
			...
			switch(color) {
				case 0 : fprintf(f,"<span class=\"b\">%10.0f</span>",mesh[i][j]);
				break;
				case 1 : fprintf(f,"<span class=\"n\">%10.0f</span>",mesh[i][j]);
				break;
				case 2 : fprintf(f,"<span class=\"g\">%10.0f</span>",mesh[i][j]);
				break;
			}
		}
		fprintf(f,"\n");
