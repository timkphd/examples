...
MPI_Status stat_ray[8];
MPI_Request req[8];
MPI_Datatype MPI_LEFT_RITE,MPI_TOP_BOT;
...
/* ! find id of neighbors using the communicators created above */
  ...
  myright = myid_row+1;if( myright  == nodes_row)myright = MPI_PROC_NULL;
    ...
    mpi_err=MPI_Bcast(&steps,1,MPI_DOUBLE,mpi_master,MPI_COMM_WORLD);
...
/* do the jacobian iterations */
    ...
    free_matrix(psi,(INT)(i1-1),(INT)(i2+1),(INT)(j1-1),(INT)(j2+1));
    free_matrix(new_psi,(INT)(i1-1),(INT)(i2+1),(INT)(j1-1),(INT)(j2+1));
    free_matrix(the_for,(INT)(i1-1),(INT)(i2+1),(INT)(j1-1),(INT)(j2+1));
...
void do_types(INT i1,INT i2,INT j1,INT j2){
	...
	num_y=j2-j1+3;
	mpi_err=MPI_Type_contiguous(num_y,MPI_DOUBLE,&MPI_TOP_BOT);
	mpi_err=MPI_Type_commit(&MPI_TOP_BOT);
	

	num_x=i2-i1+3;;
	stride=num_y;
	mpi_err=MPI_Type_vector(num_x,1,stride,MPI_DOUBLE,&MPI_LEFT_RITE);
...
/* send to left */
      if(myleft != MPI_PROC_NULL){
          mpi_err=MPI_Isend(&psi[i1-1][j1],num_x,MPI_LEFT_RITE,myleft,100,ROW_COMM,&req[ireq]);
          ireq++;
      }
/* rec from left */
      if(myleft != MPI_PROC_NULL){
        mpi_err=MPI_Irecv(&psi[i1-1][j1-1],num_x,MPI_LEFT_RITE,myleft,100,ROW_COMM,&req[ireq]);
          ireq++;
      }
/* rec from right */
      if(myright != MPI_PROC_NULL){
        mpi_err=MPI_Irecv(&psi[i1-1][j2+1],num_x,MPI_LEFT_RITE,myright,100,ROW_COMM,&req[ireq]);
          ireq++;
      }
/* send to right */
      if(myright != MPI_PROC_NULL){
        mpi_err=MPI_Isend(&psi[i1-1][j2],num_x,MPI_LEFT_RITE,myright,100,ROW_COMM,&req[ireq]);
          ireq++;
      }
    }
    else {
...
*/
    if((myid_row % 2) == 0){ 
/* send to top */
      if(mytop != MPI_PROC_NULL){
        mpi_err=MPI_Isend(&psi[i1][j1-1],  num_y,MPI_TOP_BOT,mytop,10,COL_COMM,&req[ireq]);
          ireq++;
      }
/* rec from top */
      if(mytop != MPI_PROC_NULL){
        mpi_err=MPI_Irecv(&psi[i1-1][j1-1],num_y,MPI_TOP_BOT,mytop,10,COL_COMM,&req[ireq]);  
          ireq++;
      }
/* rec from bot */
      if(mybot != MPI_PROC_NULL){
        mpi_err=MPI_Irecv(&psi[i2+1][j1-1],num_y,MPI_TOP_BOT,mybot,10,COL_COMM,&req[ireq]);
          ireq++;
      }
/* send to bot */
      if(mybot != MPI_PROC_NULL){
        mpi_err=MPI_Isend(&psi[i2][j1-1],  num_y,MPI_TOP_BOT,mybot,10,COL_COMM,&req[ireq]);
          ireq++;
      }
    }
    else{
