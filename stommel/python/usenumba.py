...
@jit("float64(float64[:,:],float64[:,:],int32,int32,int32,int32,float64,float64,float64,float64,float64,float64[:,:])")
def do_jacobi(psi,new_psi,i1,i2,j1,j2,a1,a2,a3,a4,a5,forf):
...
t1=walltime()
psi=empty(((i2-i1)+3,(j2-j1)+3),dtype=np.float64)
new_psi=empty(((i2-i1)+3,(j2-j1)+3),dtype=np.float64)
forf=empty(((i2-i1)+3,(j2-j1)+3),dtype=np.float64)
...
for i in range(0,vals.steps):
	diff=do_jacobi(psi,new_psi,i1,i2,j1,j2,a1,a2,a3,a4,a5,forf)
	if ((i+1) % iout) == 0 :
		t2=walltime()
