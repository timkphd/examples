...
! the sum of the differences between the old and new grids
    ...
    real(b8),dimension(i1-1:i2+1,j1-1:j2+1):: psi
    real(b8),dimension(i1-1:i2+1,j1-1:j2+1):: new_psi
    integer j1local ,j2local ,nt,myt
    ...
    real(b8) y
    diff=0.0_b8
!$OMP PARALLEL private (nt,myt,di,j1local ,j2local )
    nt=omp_get_num_threads()
    myt=omp_get_thread_num()
    di=real((j2-j1),b8)/real(nt,b8)
    j1local =j1+nint(di*myt)
    j2local =j1+nint(di*(myt+1))
!    write(*,*)myt,j1local,j2local
...
!                         a5*force(y)
     ...
     enddo
!$OMP END  DO
     psi(i1:i2,j1local:j2local)=new_psi(i1:i2,j1local:j2local)
!$OMP END PARALLEL
