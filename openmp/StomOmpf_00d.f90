...
program stommel
    ...
    real(b8) t1,t2,walltime,tquit,t1a,t2a
...
! do the jacobian iterations
    ...
    do i=1,steps
        call do_jacobi(psi,new_psi,diff,i1,i2,j1,j2)
	     ...
	     if(mod(i,iout) .eq. 0)then
                     !write(*,'(i6,1x,2g20.10)')i,diff,t2-t1a
               write(*,'(i6,1x,2g20.10)')i,diff,0
               t1a=t2
             endif
	     if(tquit .gt. 300.0)then
               write(*,'("did not converge",i6,1x,g20.10)')i,diff
               exit
               t2=t1
        endif
    enddo
    ...
    write(*,*)"run time =",0,n_mpi,n_thread
