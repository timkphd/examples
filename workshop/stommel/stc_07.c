...
FLT *sv1,*sv2,*sv3,*sv4,*rv1,*rv2,*rv3,*rv4;
...
/*  sv3=psi(:,j1)  */
        ...
        mpi_err=MPI_Send_init(&sv3[i1-1],num_x,MPI_DOUBLE,myleft,100,ROW_COMM,&req[ireq]);ireq++;
/* rec from left */
/*  rv3=psi(:,j1-1) */
        rv3=vector(i1-1,i2+1);
        mpi_err=MPI_Recv_init(&rv3[i1-1],num_x,MPI_DOUBLE,myleft,100,ROW_COMM,&req[ireq]);ireq++;
    }
/* rec from right */
    if(myright != MPI_PROC_NULL){
/*  rv4=psi(:,j2+1) */
        rv4=vector(i1-1,i2+1);
        mpi_err=MPI_Recv_init(&rv4[i1-1],num_x,MPI_DOUBLE,myright,100,ROW_COMM,&req[ireq]);ireq++;
/* send to right */
/*  sv4=psi(:,j2) */
        sv4=vector(i1-1,i2+1);
        mpi_err=MPI_Send_init(&sv4[i1-1],num_x,MPI_DOUBLE,myright,100,ROW_COMM,&req[ireq]);ireq++;
    }
/* send to top */
   if(mytop != MPI_PROC_NULL){
        sv1=vector(j1-1,j2+1);
/*  sv1=psi(i1,:) */
        mpi_err=MPI_Send_init(&sv1[j1-1],num_y,MPI_DOUBLE,mytop,10, COL_COMM,&req[ireq]);ireq++;
/* rec from top */
        rv1=vector(j1-1,j2+1);
/*  rv1=psi(i1-1,:) */
        mpi_err=MPI_Recv_init(&rv1[j1-1],num_y,MPI_DOUBLE,mytop,10,COL_COMM,&req[ireq]);ireq++;
    }                        
    if(mybot != MPI_PROC_NULL){
/* rec from bot */
        rv2=vector(j1-1,j2+1);
/*  rv2=psi(i2+1,:) */
        mpi_err=MPI_Recv_init(&rv2[j1-1],num_y,MPI_DOUBLE,mybot,10,COL_COMM,&req[ireq]);ireq++;
/* send to bot */
        sv2=vector(j1-1,j2+1);
/*  sv2=psi(i2,:) */
        mpi_err=MPI_Send_init(&sv2[j1-1],num_y,MPI_DOUBLE,mybot,10, COL_COMM,&req[ireq]);ireq++;
...
void do_transfer(FLT **psi,INT i1,INT i2,INT j1,INT j2){
/* ! sets the boundary conditions */
/* ! input is the grid and the indices */
/* ! for the interior cells */
    ...
    mpi_err=MPI_Startall(ireq,req);
    mpi_err=MPI_Waitall(ireq,req,stat_ray);
