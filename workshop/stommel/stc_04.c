...
void do_transfer(FLT ** psi,INT i1,INT i2,INT j1,INT j2) {
    ...
    if((myid_col % 2) == 0){
/* send to left */
      mpi_err=MPI_Isend(&svec1[i1-1],num_x,MPI_DOUBLE,myleft,100,ROW_COMM,&req[ireq]);ireq++;
/* rec from left */
      mpi_err=MPI_Irecv(&rvec1[i1-1],num_x,MPI_DOUBLE,myleft,100,ROW_COMM,&req[ireq]);ireq++;
/* rec from right */
      mpi_err=MPI_Irecv(&rvec2[i1-1],num_x,MPI_DOUBLE,myright,100,ROW_COMM,&req[ireq]);ireq++;
/* send to right */
      mpi_err=MPI_Isend(&svec2[i1-1],num_x,MPI_DOUBLE,myright,100,ROW_COMM,&req[ireq]);ireq++;
    }
    else {
...
/* rec from left */
    ...
    if((myid_row % 2) == 0){ 
/* send to top */
      mpi_err=MPI_Isend(&psi[i1][j1-1],  num_y,MPI_DOUBLE,mytop,10, COL_COMM,&req[ireq]);ireq++;  
/* rec from top */
      mpi_err=MPI_Irecv(&psi[i1-1][j1-1],num_y,MPI_DOUBLE,mytop,10,COL_COMM,&req[ireq]);ireq++;  
/* rec from bot */
      mpi_err=MPI_Irecv(&psi[i2+1][j1-1],num_y,MPI_DOUBLE,mybot,10,COL_COMM,&req[ireq]);ireq++;
/* send to bot */
      mpi_err=MPI_Isend(&psi[i2][j1-1],  num_y,MPI_DOUBLE,mybot,10, COL_COMM,&req[ireq]);ireq++;
    }
    else{
/* rec from bot */
      mpi_err=MPI_Irecv(&psi[i2+1][j1-1],num_y,MPI_DOUBLE,mybot,10,COL_COMM,&req[ireq]);ireq++;
