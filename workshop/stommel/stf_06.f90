...
!version: Uses MPI data types with MPI_TYPE_CONTIGUOUS 
!version: and MPI_TYPE_VECTOR along with MPI_SEND and MPI_RECV
...
!   include "mpif.h"
    ...
    integer req(8),ireq
    integer MPI_TOP_BOT,MPI_LEFT_RITE
end module
!*********************
program stommel
    use numz
...
! MPI_GATHERV is then used to send the data
    ...
    j3=ny+1
    if(myid .eq. mpi_master)then
        open(18,file=unique("out6d_"),recl=max(80,15*((ny)+3)+2))
...
subroutine do_types(psi,i1,i2,j1,j2)
    ...
    logical even
	num_x=i2-i1+3
	call MPI_TYPE_CONTIGUOUS(num_x,MPI_DOUBLE_PRECISION,MPI_LEFT_RITE,mpi_err)
	call MPI_TYPE_COMMIT(MPI_LEFT_RITE,mpi_err)
	num_y=j2-j1+3
	stride=num_x
	call MPI_TYPE_VECTOR(num_y,1,stride,MPI_DOUBLE_PRECISION,MPI_TOP_BOT,mpi_err)
...
! rec from left
        ireq=ireq+1
!       psi(:,j1-1)=rv3
        call MPI_IRECV(psi(i1-1,j1-1),num_x,MPI_LEFT_RITE,myleft,  &
                                      100,ROW_COMM,req(ireq),mpi_err)
    endif
...
! rec from right
        ireq=ireq+1
!       psi(:,j2+1)=rv4
        call MPI_IRECV(psi(i1-1,j2+1),num_x,MPI_LEFT_RITE,myright, &
                                      100,ROW_COMM,req(ireq),mpi_err)
! send to right
        ireq=ireq+1
!       sv4=psi(:,j2)
        call MPI_ISEND(psi(i1-1,j2),  num_x,MPI_LEFT_RITE,myright, &
                                      100,ROW_COMM,req(ireq),mpi_err)
    endif
...
! send to top
        ireq=ireq+1
!       sv1=psi(i1,:)
        call MPI_ISEND(psi(i1,j1-1),num_y,MPI_TOP_BOT,mytop,     &
                      10, COL_COMM,req(ireq),mpi_err)  
! rec from top
        ireq=ireq+1
!       psi(i1-1,:)=rv1
        call MPI_IRECV(psi(i1-1,j1-1),num_y,MPI_TOP_BOT,mytop,  &
                      10,COL_COMM,req(ireq),mpi_err)  
    endif
...
! send to bot
        ireq=ireq+1
!       sv2=psi(i2,:)
        call MPI_ISEND(psi(i2,j1-1),num_y,MPI_TOP_BOT,mybot,     &
                      10, COL_COMM,req(ireq),mpi_err)
    endif
