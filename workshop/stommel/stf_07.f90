...
!version: Uses Presistent communications with MPI_SEND_INIT and MPI_RECV_INIT
...
! of processors
  ...
  if(myid .eq. mpi_master) write(*,*)" nrow= ",nrow," ncol= ",ncol
...
! myid=   5 myrow=   2 mycol=   3    ( 26 <= i <=  50) ,  ( 34 <= j <=  50)
    ...
    if(myid == mpi_master)write(*,'("nodes_row= ",i4," nodes_col= ",i4)')nodes_row,nodes_col
...
! set up the persistent requests
    call do_presistent(psi,i1,i2,j1,j2)
...
! MPI_GATHERV is then used to send the data
    ...
    j3=ny+1
    if(myid .eq. mpi_master)then
        open(18,file=unique("out7d_"),recl=max(80,15*((ny)+3)+2))
...
subroutine do_presistent(psi,i1,i2,j1,j2)
    use numz
    use mympi
    use input
    implicit none
...
!        rv4=psi(:,j2+1)
        call MPI_RECV_INIT(rv4,num_x,MPI_DOUBLE_PRECISION,myright, &
                           100,ROW_COMM,req(ireq),mpi_err)
! send to right
        ireq=ireq+1
        allocate(sv4(i1-1:i2+1))
!       sv4=psi(:,j2)
        call MPI_SEND_INIT(sv4,  num_x,MPI_DOUBLE_PRECISION,myright, &
                           100,ROW_COMM,req(ireq),mpi_err)
    endif
!    goto 1
! send to top
...
!        sv1=psi(i1,:)
        call MPI_SEND_INIT(sv1,num_y,MPI_DOUBLE_PRECISION,mytop,     &
                           10, COL_COMM,req(ireq),mpi_err)
! rec from top
        ireq=ireq+1
        allocate(rv1(j1-1:j2+1))
!        rv1=psi(i1-1,:)
        call MPI_RECV_INIT(rv1,num_y,MPI_DOUBLE_PRECISION,mytop,  &
                           10,COL_COMM,req(ireq),mpi_err)
    endif                        
...
!        rv2=psi(i2+1,:)
        call MPI_RECV_INIT(rv2,num_y,MPI_DOUBLE_PRECISION,mybot,  &
                           10,COL_COMM,req(ireq),mpi_err)
! send to bot
        ireq=ireq+1
        allocate(sv2(j1-1:j2+1))
!       sv2=psi(i2,:)
        call MPI_SEND_INIT(sv2,num_y,MPI_DOUBLE_PRECISION,mybot,     &
                           10, COL_COMM,req(ireq),mpi_err)

    endif
 1 continue
    write(*,*)"ireq=",ireq
end subroutine do_presistent
!*********************
subroutine do_transfer(psi,i1,i2,j1,j2)
! sets the boundary conditions
! input is the grid and the indices for the interior cells
