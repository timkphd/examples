...
! the sum of the differences between the old and new grids
    ...
    real(b8),dimension(i1-1:i2+1,j1-1:j2+1):: psi
    real(b8),dimension(i1-1:i2+1,j1-1:j2+1):: new_psi
    integer i1local,i2local,nt,myt
    ...
    real(b8) y
    diff=0.0_b8
!$OMP PARALLEL private (nt,myt,di,i1local,i2local)
    nt=omp_get_num_threads()
    myt=omp_get_thread_num()
    di=real((i2-i1),b8)/real(nt,b8)
    i1local=i1+nint(di*myt)
    i2local=i1+nint(di*(myt+1))
...
!                         a5*force(y)
     ...
     enddo
!$OMP END  DO
     psi(i1local:i2local,j1:j2)=new_psi(i1local:i2local,j1:j2)
!$OMP END PARALLEL
